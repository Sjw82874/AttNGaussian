...
# scaling_modifier 点云缩放因子, override_color 覆盖点云颜色, feature_linear 计算特征图
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, feature_linear = None):
    ...
    tanfovx = math.tan(viewpoint_camera.FoVx * 0.5)  #计算场视角
    ...
    if pipe.compute_cov3D_python:
        cov3D_precomp = pc.get_covariance(scaling_modifier)  #计算3D协方差，用于计算高斯点形状
    else:
    ...
    shs = None
    colors_precomp = None
    if feature_linear is None:  #是否是特征图
        if override_color is None:  #是否覆盖点云颜色
            if pipe.convert_SHs_python:  #计算球谐函数表示的颜色
