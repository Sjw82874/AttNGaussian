...
from arguments import ModelParams, PipelineParams, get_combined_args  # 将cfg_args与命令行的参数合并
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, style=None):
    ...
    if style:
        ...
        render_path = os.path.join(model_path, name, style_name, "render")
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):  # 获取每个视角及其对应索引
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, style_img_path, skip_train : bool, skip_test : bool):
    with torch.no_grad():
        ...
        if style_img_path:
            ...
            scene = Scene(dataset, gaussians, load_path=ckpt_path, shuffle=False, style_model=True)  # 不打乱相机视角顺序

            # read style image
            trans = T.Compose([T.Resize(size=(256,256)), T.ToTensor()])
            style_img = trans(Image.open(style_img_path)).cuda()[None, :3, :, :]  # 增加batch维度，且只取RGB通道
            style_name = Path(style_img_path).stem  # 获取风格图片名
...
def render_sets_style_interpolate(dataset : ModelParams,  pipeline : PipelineParams, style_img_paths, view_id=0):
    with torch.no_grad():
        ...
        v = torch.linspace(0,1,steps=5)  # 在[0,1]中等间隔取5个值
        up_maps = []
        for i in range(5):
            up_maps.append(tranfered_features0 * v[i] + tranfered_features1 * v[4-i])  # 第一行，根据权重插值
        down_maps = []
        for i in range(5):
            down_maps.append(tranfered_features2 * v[i] + tranfered_features3 * v[4-i])  # 最后一行，根据权重插值

        images = []
        w = torch.linspace(0,1,steps=4)
        for y in range(4):  # 根据列生成插值图像
            for x in range(5):
                ...
                view = scene.getTrainCameras()[view_id]  # 默认用第一张视角图生成插值图像
                rendering = render(view, gaussians, pipeline, background, override_color=override_color)["render"]
                rendering = rendering.clamp(0, 1)

                rendering = torchvision.transforms.functional.resize(rendering, 300)  # 将风格化图像大小调整为300x300

                images.append(rendering)

        torchvision.utils.save_image(images, fp=f'{render_path}/{all_style_name}.png', nrow=5, padding=0)  # 图像间没有空隙
...
def render_sets_content_interpolate(dataset : ModelParams,  pipeline : PipelineParams, style_img_path, view_id=0):
    with torch.no_grad():
        ...
        images = []
        for x in range(5):
            tranfered_features_interpolated = tranfered_features * v[x] + gaussians.final_vgg_features * v[4-x]  # 不同风格迁移程度
...
if __name__ == "__main__":
    ...
    model = ModelParams(parser, sentinel=True)  # 模型参数需显式指定，因此需要cfg_args
    pipeline = PipelineParams(parser)
    parser.add_argument("--iteration", default=-1, type=int)  # 没用到
