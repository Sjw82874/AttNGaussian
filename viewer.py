...
class ViserViewer:
    def __init__(self, gaussians, pipeline, background, override_color, training_cams, wikiart_img_paths=None, viewer_port='8080'):
        ...
        with self.server.gui.add_folder("Rendering Settings"):
            ...
            self.jpeg_quality_slider = self.server.gui.add_slider(
                "JPEG Quality", min=0, max=100, step=1, initial_value=80
            )

            self.training_view_slider = self.server.gui.add_slider(
                "Training View",
                min=0,
        ...
        with self.server.gui.add_folder("Style Transfer"):
            self.style_img_path_text = self.server.gui.add_text(
                    "Style Image",
                    initial_value="",
                    hint="Path to style image",                
                )
            
            self.display_style_img = self.server.gui.add_checkbox("Display Style Image", initial_value=True)

            if wikiart_img_paths is not None:
                self.random_style_button = self.server.gui.add_button("Random Style")

        
        with self.server.gui.add_folder("Style Interpolation"):
            self.style_path_1 = self.server.gui.add_text(
                "Style 1",
                initial_value="",
                hint="Path to style image",
            )

            self.style_path_2 = self.server.gui.add_text(
                "Style 2",
                initial_value="",
                hint="Path to style image",
            )

            self.interpolation_ratio = self.server.gui.add_slider(
                "Interpolation Ratio",
                min=0,
                max=1,
                step=0.01,
